...
class Conv1DAutoencoder:
    def __init__(self, n_visible=5, n_hidden=3, lr=0.001, corruption_level=0.0, hidden_ratio=None, sequence_length=500,
                 seed=1234, ar=False, haste: bool = False, name="Anonymous dA"):
        """
        ...
        :param haste: If True, the model will train for 1 epoch.
        ...
        self.encoder = tf.keras.Sequential([
            ...
            tf.keras.layers.Conv1D(filters=n_visible, kernel_size=sequence_length // 4, padding='same', activation='tanh'),
            tf.keras.layers.MaxPooling1D(pool_size=2, padding='same'),
            tf.keras.layers.Conv1D(filters=self.n_hidden, kernel_size=sequence_length // 4, padding='same', activation='relu'),
            tf.keras.layers.GlobalAveragePooling1D(), # Reduce to vector
        ...
        self.model = tf.keras.models.Sequential([self.encoder, self.decoder], name='Conv1DAutoencoder')
