...
class TestKitNETEndToEnd:
    ...
    def test_elm_produces_scores(self):
        ...
        scores = kn.run(X)
        assert len(scores) == 500  # n_samples - fm_grace - ad_grace
    ...
    def test_elm_detects_anomaly(self):
        ...
        kn = KitNET(n_features=20, ae_type='elm', fm_grace=500, ad_grace=500)
        scores = kn.run(X)
        # Execution starts at sample 1000; attack starts at 1200
        # So in scores: benign = [:200], attack = [200:]
